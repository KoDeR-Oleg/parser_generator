...
class TestYandexParser(unittest.TestCase):
    ...
    def test_filename_in_markup_page1(self):
        ...
        self.assertEqual(markup.file, "test/yandex/1.html")

    def test_filename_in_markup_page2(self):
        parser = YandexParser()
        markup = parser.extract_markup("test/yandex/2.html")
        self.assertEqual(markup.file, "test/yandex/2.html")
    ...
    def test_markups_of_first_result_search_on_page1(self):
        parser = YandexParser()
        markup_component = parser.extract_markup("test/yandex/1.html").components[0]
    ...
    def test_markup_regress_on_page1(self):
        parser = YandexParser()
        markup = parser.extract_markup("test/yandex/1.html")
        with open("test/yandex/1_markup.json", "r") as file:
    ...
    def test_markup_regress_on_page2(self):
        parser = YandexParser()
        markup = parser.extract_markup("test/yandex/2.html")
        with open("test/yandex/2_markup.json", "r") as file:
            expected_markup = file.read()
        self.assertEqual(str(markup), expected_markup)
